...
class NNModel:
    ...
    def build(self):
        ...
        pnl = Input(shape=(1,))
        time_to_maturity = Input(shape=(1,))
        inputs = [pnl, time_to_maturity, hedging_inst]
        layers = []
        ...
        for t in range(self._n_points):
            layer_input = Concatenate(axis=1)([hedging_inst, time_to_maturity])
            for i in range(self._layers - 1):
            ...
            if self._n_hedging_inst > 1:
                pnl_t = Lambda(lambda x: K.sum(x, axis=1))(Multiply()([strategy, change_in_price]))
            else:
                pnl_t = Multiply()([strategy, change_in_price])
            pnl = Add()([pnl_t, pnl])
            time_to_maturity = Lambda(lambda x: x + self._dt)(time_to_maturity)
    ...
    def train(self, loss_name='mean_squared_error', lam=None, batch_size=32, epochs=5, lr=None, n_cycle=1, n_paths=10 ** 5):
        ...
        self._x_train, self._y_train = dataset.generate_dataset(self._hedging_inst)
    ...
    def build_hist(self, bins=100, xmin=-0.03, xmax=0.03, save=False):
        ...
        if save:
            fig_name = self._derivative_name + '_pnl_distribution_' + self._loss_name
            if self._loss_name == 'exponential_utility':
    ...
    def get_sample_paths(self, x_test=None, n_paths=10, save=False):
        ...
        if not x_test:
            ...
            x_test, y_test = dataset.generate_dataset(self._hedging_inst)
        else:
        ...
        for t in range(self._n_points):
            sub_model_input = [x_test[t + 2], t * self._dt * np.ones((n_paths, 1))]
            strategies.append(sub_model.predict(sub_model_input))
        ...
        if save:
            fig_name = self._derivative_name+'_strategy_sample_paths_'+self._loss_name
            if self._loss_name == 'exponential_utility':
                fig_name += '_'+str(self._lam)
            fig_name += '.png'
            fig.savefig(fig_name)
    ...
    def build_delta(self, n=40, save=False):
        ...
        if save:
            fig_name = self._derivative_name+'_delta_'+self._loss_name
            if self._loss_name == 'exponential_utility':
                fig_name += '_'+str(self._lam)
            fig_name += '.png'
            fig.savefig(fig_name)
    ...
    def build_delta_gamma(self, n=100, save=False):
        ...
        if save:
            fig_name = self._derivative_name+'_delta_gamma_'+self._loss_name
            if self._loss_name == 'exponential_utility':
                fig_name += '_'+str(self._lam)
            fig_name += '.png'
            fig.savefig(fig_name)
